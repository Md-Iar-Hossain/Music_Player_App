  ...
  */
      ...
      allTrackList = document.getElementById('all_track_list'),
      favouriteList = document.getElementById('favourite_list'),
      audioTrackContainer = document.querySelector('.audio_track_container'),
      favouriteTrackContainer = document.querySelector('.favourite_track_container'),
      ...
      audioTrackObject.forEach((obj, index) => {
        ...
        <div style="--totalIndex: ${audioTrackObject.length}" class="audio_track_item d-flex" data-title="${obj.trackTitle}" data-img="${obj.img}" data-url="${obj.url}">
      ...
      favouriteList.addEventListener('click',showFavouriteTrackList)
      ...
      function showAllTrackList() {
        ...
        favouriteTrackContainer.style.display = 'none'
      }
      function showFavouriteTrackList() {
        audioTrackContainer.style.display = 'none'
        favouriteTrackContainer.style.display = 'block'
      }
      // keyboard navigator
      document.addEventListener('keydown',(e)=> {
        if (!keyboardShortcutPane.classList.contains('show')) {
          (e.key === "e")? trackPlayerTray.classList.add('expand'): 
          (e.key === "Escape")? trackPlayerTray.classList.remove('expand') :
          (e.code === "Space")? playOrPauseSong(1) : 
          (e.key === "ArrowRight")? playNextSong() : 
          (e.key === "ArrowLeft")? playPrevSong() : null;
        }
        if (trackPlayerTray.classList.contains('expand')) {
          (e.key === "s")? playRandomSong():
          (e.key === "r")? musicLoop() : null
        }
      })
